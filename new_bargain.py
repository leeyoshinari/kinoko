...
def query_code(ms_code, hospital, res: dict):
    try:
        url = f"{host2}/tps_local_bd/web/mcstrans/mcsBargain/getMcsBargainRecordList"
        data = {"current": 1, "size": 10, "searchCount": True, "searchTime": [], "status": "1", "pubonlnRsltIdYj": str(ms_code), "medinsName": hospital}
        response = session.post(url, json=data, headers=headers)
        if response.status_code == 200:
            if '重新登录' in response.text and '其他设备' in response.text:
                os.remove(cookie_path)
                raise Exception('有其他设备已登录，请重新登录')
            if 'token过' in response.text:
                os.remove(cookie_path)
                raise Exception('已过期，请重新登录')
            res_json = json.loads(response.text)
            if res_json['code'] == 0 and res_json['data'] and len(res_json['data']['records']) == 1:
                res.update({"amount": res_json['data']['records'][0]['amount']})
                res.update({"bargainDeadline": res_json['data']['records'][0]['bargainDeadline']})
                res.update({"id": res_json['data']['records'][0]['id']})
                res.update({"negotiatingPrice": res_json['data']['records'][0]['negotiatingPrice']})
                res.update({"sellerRepDeadline": res_json['data']['records'][0]['sellerRepDeadline']})
                return res
            else:
                logger.error(f"议价列表查询结果为空或有多个，药交耗材Id：{ms_code}，医疗机构：{hospital}，查询结果：{res_json['data']}")
                raise
        else:
            logger.error(f"议价列表查询失败，药交耗材Id：{ms_code}，医疗机构：{hospital}，状态码：{response.status_code}")
            raise
    except:
        raise
...
def agree_bargain(agree_val, res: dict):
    try:
        url = f'{host2}/tps_local_bd/web/mcstrans/mcsBargain/modifyMcsBargainById'
        if agree_val == '同意':
            res.update({'operatingContent': '企业同意医院议价申请'})
            res.update({'status': 3})
        elif '核实价格' in agree_val:
            res.update({'operatingContent': '企业拒绝医院议价申请'})
            res.update({'remark': agree_val})
            res.update({'status': 4})
        else:
            try:
                _ = float(agree_val)
            except:
                raise Exception(f"议价执行填写不正确：当前值 {agree_val}")
            res.update({'negotiatingPrice': agree_val})
...
try:
    ...
    for file_name in file_names:
        ...
        for i in range(ind, table.nrows):
            ...
            total_num += 1
            try:
                ms_code = table.cell_value(i, 2).strip()
            except:
                ms_code = str(int(table.cell_value(i, 2)))
            hospital = table.cell_value(i, 1).strip()
            is_agree = table.cell_value(i, 3)
            if ms_code and hospital and is_agree:
                try:
                    time.sleep(2)
                    res = {}
                    res = query_code(ms_code, hospital, res)
                    agree_bargain(is_agree, res)
                    success += 1
                    logger.info(f"议价成功，药交ID：{ms_code}，医疗机构：{hospital}，议价执行：{is_agree}")
                except:
                    logger.error(traceback.format_exc())
                    logger.error(f"议价失败，药交ID：{ms_code}，医疗机构：{hospital}，议价执行：{is_agree}")
            else:
                logger.error(f"Excel表格中的数据不全，药交ID：{ms_code}，医疗机构：{hospital}，议价执行：{is_agree}")
